...
app.controller("menuAddAndUpdateController", ['$rootScope','$scope','$routeParams','$mp_ajax','$location','$mp_json','$window',function($rootScope,$scope,$routeParams,$mp_ajax,$location,$mp_json,$window) {
    ...
    console.log("menu_add_update_controller.js");
...
//    $scope.formURL = "";
    ...
    if(!angular.isUndefined($scope.typeId) && angular.isUndefined($scope.menuItemId)) {  //For create new menu item
        ...
        $scope.title = "Create new menu item";
        ...
        $scope.onSubmit = function(menuItem) {
...
//            console.debug(tmpObj);
            $mp_ajax.put('/biz/'+$scope.bizId+'/prod/'+$scope.menuItemId, tmpObj,function(data){
                if(data.succeed==true) {
                    $location.path("/menu");
//                    console.debug("image path = "+angular.element("#menu_add_image").val());
//                    var imgPath = angular.element("#menu_add_image").val();
//                    if(imgPath!="") { // image file selected or not
//                        //step 2 : image [form]
//                        $mp_ajax.formPost($('#menuItemForm'), '../biz/' + $scope.bizId + '/prod/' + $scope.menuItemId + '/image',
//                            function (data) {
//                                console.log("uploadimg ok , path="+data);
//                                $window.location.href = "#/menu";
////                                $location.path("/menu");
////                                $scope.$apply(); //otherwise , menu page can't be loaded
//                            }, function (data) {
//                                console.error("update menu item image error, msg = " + data.responseJSON.message);
//                            }
//                        );
//                    }
//                    else {
//                        console.debug("no img selected");
//                        $location.path("/menu");
//                    }
                }
            },function(data){
...
//            title_html: true,
    ...
    console.log('000',$scope.menuItem.img_url);
    uploadMenuItemImage($scope,$mp_ajax);
    //move page-content a little bit down in case of tabs cover part of it
    OnViewLoad();
}] );

function uploadMenuItemImage($scope,$mp_ajax){
    console.log('111',$scope.menuItem.img_url);
    ...
    try {//ie8 throws some harmless exception, so let's catch it
        ...
        $('#avatar').editable({
            ...
            image: {
                ...
                btn_choose: 'Click me to select a picture',
                droppable: true,
                thumbnail:'fit',
